...
|---|---|---|---|---|
| 2026-09-23 | First pilot connect attempt | "While it is saying Approve the request in Lace, nothing pops up" | Caused by the row below: browsers only open an extension window from a click, so timer retries asked Lace for a session when no prompt could appear. Reverted to one attempt per click, and the dialog now says the request is waiting behind the Lace toolbar icon | `98126c4` |
| 2026-09-23 | First pilot connect attempt | "I have entered my password about 2 times per connection before the timeout": every refusal while Lace was locked ended the attempt | First attempt waited through the lock on a timer (`5a689b5`), which caused the row above and was reverted. The lock is now named plainly and the retry is a click: "Lace is locked. Unlock it, then press Connect again" | `98126c4` |
